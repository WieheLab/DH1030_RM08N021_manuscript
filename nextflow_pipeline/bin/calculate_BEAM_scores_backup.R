...
antigens <- colnames(all)[grepl('BEAM[01-16]', colnames(all))]
...
scores <- scores %>% mutate_at(antigens, as.numeric)
...
colnames(all_with_scores) <- mgsub(pattern = antigen_names$id, replacement = antigen_names$name, text.var = colnames(all_with_scores)) 
...
all_with_scores_and_seqs <- select(all_with_scores_and_seqs, ConvertedID, barcode, Fxnl_1to1, Converted_ReadID_Heavy, ReadID_Heavy, CloneID_Heavy:MuFreq_Heavy, Converted_ReadID_Light, ReadID_Light, CloneID_Light:Light_VJ_Seq)

order <- select(all_with_scores_and_seqs, grep('_BEAM_Score', colnames(all_with_scores_and_seqs), value = T)) %>% apply(., 1, sum) %>% order(., decreasing = T)
...
write.csv(all_with_scores_and_seqs, file = paste0(Exp, '_', PTID, '_merged_clonal_analysis.fxnl_1to1.BEAM_Scores.UMIs.Seqs.csv'), row.names = FALSE, quote = FALSE)

# Get long format for ggplot input
cols <- colnames(all_with_scores)[grepl('_BEAM_Score', colnames(all_with_scores))]

heatmap_sub <- select(all_with_scores, barcode, all_of(cols))
...
heatmap_long$Antigen <- factor(heatmap_long$Antigen, levels = antigen_names$name)

CairoPDF(paste0(Exp, '_', PTID, ".heatmap.pdf"), height = 8, width = 28)
...
row_sums <- apply(heatmap_sub[, ..numeric_cols, with = FALSE], 1, sum)

# Subset based on condition that row sums is greater than 10 or > nrow(feature_reference)
nonzero_barcodes <- heatmap_sub[row_sums > nrow(antigen_names), barcode]
...
if (length(nonzero_barcodes) > 0 ) {
  ...
  CairoPDF(heatmap_out_trimmed_name, height = 8, width = 28)
  ggplot(heatmap_long_sub, aes(x = Barcode, y = Antigen, fill = BEAM_Score)) + geom_tile() + 
    scale_fill_gradient(low = 'grey90', high = 'red', limits = c(0,100)) + 
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.title = element_text(hjust = .5)) + 
    xlab('Barcode') + 
    ylab('Antigen') + labs(fill = 'BEAM Score') + scale_y_discrete(expand = c(0,0)) + scale_x_discrete(expand = c(0,0)) 
