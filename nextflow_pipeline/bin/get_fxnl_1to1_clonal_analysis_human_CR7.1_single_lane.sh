...
mono ${CLOANALYST_PATH}/Cloanalyst.dll parse  --thread $num_threads -s "Homo sapiens" -c heavy --excl 5 -g "AR20170307 FPC-F" heavies.fasta
#time mpirun -np $num_threads /opt/apps/rhel7/cloanalyst/cloanalyst_mpi-new parse --thread $num_threads -s "\"Homo sapiens\"" -c heavy --excl 5 -g "\"AR20170307 FPC-F\"" heavies.fasta
/hpc/group/dhvi/WieheLab/scripts/Compiled/VDJ_trimmer/VDJ_trimmer -SMUA heavies.SimpleMarkedUAs.fasta -species human -chain heavy > heavies.VDJ.fasta
...
mono ${CLOANALYST_PATH}/Cloanalyst.dll parse  --thread $num_threads -s "Homo sapiens" -c kappa --excl 5 -g "AR20170307 FPC-F" kappas.fasta
#time mpirun -np $num_threads /opt/apps/rhel7/cloanalyst/cloanalyst_mpi-new parse --thread $num_threads -s "\"Homo sapiens\"" -c kappa --excl 5 -g "\"AR20170307 FPC-F\"" kappas.fasta
/hpc/group/dhvi/WieheLab/scripts/Compiled/VDJ_trimmer/VDJ_trimmer -SMUA kappas.SimpleMarkedUAs.fasta -species human -chain kappa > kappas.VJ.fasta
...
#time mpirun -np $num_threads /opt/apps/rhel7/cloanalyst/cloanalyst_mpi-new parse --thread $num_threads -s "\"Homo sapiens\"" -c lambda --excl 5 -g "\"AR20170307 FPC-F\"" lambdas.fasta
mono ${CLOANALYST_PATH}/Cloanalyst.dll parse  --thread $num_threads -s "Homo sapiens" -c lambda --excl 5 -g "AR20170307 FPC-F" lambdas.fasta
/hpc/group/dhvi/WieheLab/scripts/Compiled/VDJ_trimmer/VDJ_trimmer -SMUA lambdas.SimpleMarkedUAs.fasta -species	human -chain lambda > lambdas.VJ.fasta
...
cd ../
Rscript /datacommons/dhvi/mb488/scripts/10x/R/merge_clone_info.R -I ../cellranger_output/all_contig_annotations.umi_filtered.csv
