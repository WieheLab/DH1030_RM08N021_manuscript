...
job_id=$(sbatch /hpc/group/dhvi/WieheLab/scripts/partis/job_scripts/partis_flexible_annotate.job \
-p heavies.fasta \
-c igh \
-g $GERMLINE \
-w $PARTIS_WORKDIR \
...
python3 /hpc/group/dhvi/WieheLab/scripts/partis/python_scripts/partis_to_RS_IMGT_hashed_wErr.py -p heavies.partis.annotate.yaml -c heavy -s human -r 1 -n 0
...
# Convert to SMUA, Recombination Summaries Files
python3 /hpc/group/dhvi/WieheLab/scripts/partis/python_scripts/partis_to_RS_IMGT_hashed_wErr.py -p kappas.partis.annotate.yaml -c kappa -s human -r 1 -n 0
...
# Convert to SMUA, Recombination Summaries Files
python3 /hpc/group/dhvi/WieheLab/scripts/partis/python_scripts/partis_to_RS_IMGT_hashed_wErr.py -p lambdas.partis.annotate.yaml -c lambda -s human -r 1 -n 0

# VDJ trimming
Rscript /hpc/group/dhvi/WieheLab/scripts/R/scripts/vdjtrimmer.R -s VL_SimpleMarkedUAs.fasta -o lambdas.VJ.fasta

LAMBDA_FASTA=$(readlink -f heavies.fasta)
...
-w $PARTIS_WORKDIR \
-t 10 \
-m $PARTIS_HEAVY_OUT | awk '{print $4}')
...
-w $PARTIS_WORKDIR \
-t 10 \
-m $PARTIS_KAPPA_OUT | awk '{print $4}')
...
-w $PARTIS_WORKDIR \
-t 10 \
-m $PARTIS_LAMBDA_OUT | awk '{print $4}')
...
cd ../
Rscript /hpc/group/dhvi/WieheLab/scripts/nextflow_pipelines/10x-nextflow-pipeline/bin/partis_merge_clone_info.R ../cellranger_output/all_contig_annotations.umi_filtered.csv

cd ../
Rscript /hpc/group/dhvi/WieheLab/scripts/nextflow_pipelines/10x-nextflow-pipeline/bin/partis_subset_rs_from_merged_clonal_analysis.R
