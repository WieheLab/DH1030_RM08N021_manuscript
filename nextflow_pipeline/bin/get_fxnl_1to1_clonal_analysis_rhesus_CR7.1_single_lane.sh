...
# Move cellranger files and split cellranger output by chain for input into Cloanalys
...
#time mpirun -np $num_threads /opt/apps/rhel7/cloanalyst/cloanalyst_mpi-new parse --thread $num_threads -s "\"Macaca mulatta\"" -c heavy --excl 5 -g "\"AR20170307 FPC-F\"" heavies.fasta
mono ${CLOANALYST_PATH}/Cloanalyst.dll parse  --thread $num_threads -s "Macaca mulatta" -c heavy --excl 5 -g "AR20170307 FPC-F" heavies.fasta
/hpc/group/dhvi/WieheLab/scripts/Compiled/VDJ_trimmer/VDJ_trimmer -SMUA heavies.SimpleMarkedUAs.fasta -species rhesus -chain heavy > heavies.VDJ.fasta
...
#time mpirun -np $num_threads /opt/apps/rhel7/cloanalyst/cloanalyst_mpi-new parse --thread $num_threads -s "\"Macaca mulatta\"" -c kappa --excl 5 -g "\"AR20170307 FPC-F\"" kappas.fasta
mono ${CLOANALYST_PATH}/Cloanalyst.dll parse  --thread $num_threads -s "Macaca mulatta" -c kappa --excl 5 -g "AR20170307 FPC-F" kappas.fasta
/hpc/group/dhvi/WieheLab/scripts/Compiled/VDJ_trimmer/VDJ_trimmer -SMUA kappas.SimpleMarkedUAs.fasta -species rhesus -chain kappa > kappas.VJ.fasta
...
#time mpirun -np $num_threads /opt/apps/rhel7/cloanalyst/cloanalyst_mpi-new parse --thread $num_threads -s "\"Macaca mulatta\"" -c lambda --excl 5 -g "\"AR20170307 FPC-F\"" lambdas.fasta
mono ${CLOANALYST_PATH}/Cloanalyst.dll parse  --thread $num_threads -s "Macaca mulatta" -c lambda --excl 5 -g "AR20170307 FPC-F" lambdas.fasta
/hpc/group/dhvi/WieheLab/scripts/Compiled/VDJ_trimmer/VDJ_trimmer -SMUA lambdas.SimpleMarkedUAs.fasta -species	rhesus -chain lambda > lambdas.VJ.fasta
...
cd ../
Rscript /datacommons/dhvi/mb488/scripts/10x/R/merge_clone_info.R --filtered_contig_annotations ../cellranger_output/all_contig_annotations.umi_filtered.csv
